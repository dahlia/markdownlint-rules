...
name: release
description: >-
  Create and publish releases for the @hongminhee/markdownlint-rules package.
...
1.  Check if a newer maintenance branch exists (e.g., `0.2-maintenance`):
...
3.  If no newer maintenance branch exists, merge to `main`:
    ...
    ~~~~ bash
    git checkout main
    git merge 0.1.1  # or the last tag you created (e.g., 0.2.1)
    ~~~~
...
4.  Bump the version in *deno.json*:
...
------------------------
 ...
 -  Patch releases: `X.Y.Z` where Z > 0 (e.g., 0.1.1, 0.1.2)
 -  Minor releases: `X.Y.0` (e.g., 0.2.0, 0.3.0)
 -  Major releases: `X.0.0` (e.g., 1.0.0, 2.0.0)
 -  Maintenance branches: `X.Y-maintenance` (e.g., 0.1-maintenance)
 -  Tags: No `v` prefix (e.g., `0.1.1`, not `v0.1.1`)
 -  Tag messages: `@hongminhee/markdownlint-rules X.Y.Z` format (use `-m` flag
    to avoid editor)
...
- [ ] Create tag `X.Y.Z` with `-m "@hongminhee/markdownlint-rules X.Y.Z"`
...
- [ ] Create tag `X.Y.0` with `-m "@hongminhee/markdownlint-rules X.Y.0"`
- [ ] Add next version section to *CHANGES.md*
- [ ] Bump version in *deno.json*
- [ ] Run `deno task check-versions --fix`
- [ ] Commit with message "Version bump\n\n[ci skip]"
...
- [ ] Bump version to X.Y.1 in *deno.json*
- [ ] Run `deno task check-versions --fix`
- [ ] Commit with message "Version bump\n\n[ci skip]"
